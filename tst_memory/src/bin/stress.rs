...
type KeyGenerator = fn(usize) -> Vec<Vec<u8>>;
...
fn test_22_pathological() -> Vec<TestResult> {
    ...
    for (i, k) in baseline_keys.iter().enumerate() {
        baseline_tst.insert(k, i as u32);
    }
    ...
    let distributions: &[(&str, KeyGenerator)] = &[
...
fn test_23_arena_fragmentation() -> TestResult {
    ...
    for _ in 0..n_ops {
        if xorshift64(&mut state).is_multiple_of(2) && !live_keys.is_empty() {
    ...
    TestResult {
        ...
        metrics: Metrics {
            ...
            extra: format!(
                "ops={} ins={} del={} survivors={} churn_ms={}",
                n_ops,
                inserts,
                deletes,
                live_keys.len(),
...
fn test_25_decay_boundary() -> Vec<TestResult> {
    ...
    results.push(TestResult {
        ...
        metrics: Metrics {
            ...
            ops_per_sec: 0,
            extra: format!(
                "alpha={:.3} predicted_cycles={} actual_cycles={} final_score={:.6}",
                alpha, predicted, cycles, score
            ),
        },
    });
    ...
    results.push(TestResult {
        ...
        metrics: Metrics {
            ...
            ops_per_sec: 0,
            extra: format!(
                "max_bias_seen={:.4} clamp_max={:.4}",
                max_bias, config.bias_clamp_max
            ),
        },
    });
    ...
    let no_nan_inf = !bias_nan.is_nan()
        && !bias_nan.is_infinite()
        && !bias_inf.is_nan()
        && bias_inf <= config.bias_clamp_max;
    results.push(TestResult {
...
fn test_31_stm_saturation() -> TestResult {
    ...
    for i in recent_start..(CAP + EXTRA) {
        if stm.lookup_mut(i.to_string().as_bytes()).is_some() {
            hits += 1;
        }
    }
    ...
    TestResult {
        ...
        metrics: Metrics {
            ...
            ops_per_sec: 0,
            extra: format!(
                "recent_hits={}/{} stale_hits={} extra_writes={}",
                hits, CAP, stale_hits, EXTRA
            ),
        },
    }
}
...
fn test_82_bias_clamp() -> Vec<TestResult> {
    ...
    for &(label, clamp_min, clamp_max) in clamp_configs {
        let config = ModelConfig {
            bias_scale: 1.0,
            bias_clamp_min: clamp_min,
            bias_clamp_max: clamp_max,
        };
        ...
        for &freq in &freqs {
            for &score in &scores {
                ...
                if b < clamp_min - 1e-6 || b > clamp_max + 1e-6 {
                    any_oob = true;
                }
            }
        }
...
fn test_91_persistence() -> TestResult {
    ...
    for i in 0..n {
        ...
        if let Some(p) = kernel2.route_read(key.as_bytes())
            && let PayloadData::Preference { value, .. } = &p.data
            && *value == format!("val_{}", i)
        {
            recalled += 1;
        }
    ...
    TestResult {
        ...
        metrics: Metrics {
            ...
            ops_per_sec: 0,
            extra: format!(
                "recalled={}/{} save_us={} load_us={} snap_bytes={}",
                recalled, n, save_us, load_us, snap_bytes
            ),
        },
    }
}
...
fn main() {
    let tree_only = std::env::args()
        .skip(1)
        .any(|argument| argument == "--tree-only");
    let sep = "─".repeat(100);
    println!("{}", sep);
