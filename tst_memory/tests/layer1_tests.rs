...
use tst_memory::tree::{NodeType, TreeEvent, TreeMemory};
...
fn test_layer1_stm_isolation() {
    ...
    let _ = stm.lookup_mut(b"5");
    let lookup_time = start_time.elapsed();
    assert!(lookup_time.as_millis() < 1, "Lookup exceeded 1ms");

    // Decay test
    let initial_score = stm.lookup_mut(b"5").unwrap().reinforcement_score;
    stm.decay_tick();
    let decayed_score = stm.lookup_mut(b"5").unwrap().reinforcement_score;
    assert_eq!(decayed_score, initial_score * 0.98);

    // Promotion test
    for _ in 0..10 {
        let entry = stm.lookup_mut(b"5").unwrap();
    ...
    assert!(
        small_stm.lookup_mut(b"1").is_none(),
        "Oldest entry was not evicted"
    );
    assert!(
        small_stm.lookup_mut(b"5").is_some(),
        "Newest entry not found"
    );
}
...
fn test_layer1_tree_isolation() {
    ...
    let f1 = tree
        .process_event(TreeEvent::FileAdded {
            parent_id: pid,
            name: "file1.py".to_string(),
        })
        .unwrap();
